...
noncomputable def {alpha_name}_minpoly : Polynomial ℂ :=
  Polynomial.ofFinsupp (Finsupp.mk (List.range {d + 1}).toFinset
    (fun i => match i with
{chr(10).join(f"      | {i} => {fmt_rat(c)}" for i, c in enumerate(minpoly_coeffs))}
      | _ => 0)
    (by decide))
...
def emit_coeff_decomp(weight, n_pieces, alpha_name):
    ...
    n_smuls = n_pieces - 1  # f_0 isn't smul'd
    rw_lines = []
...
as `c_0 + c_1 · α + c_2 · α² + … + c_{{{n_pieces - 1}}} · α^{n_pieces - 1}`, giving {n_pieces}
rational pieces. The combined form is `f = f_0 + α · f_1 + … + α^{n_pieces - 1} · f_{{{n_pieces - 1}}}`.
