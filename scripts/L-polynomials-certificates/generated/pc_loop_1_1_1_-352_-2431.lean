...
def E : WeierstrassCurve ℤ := ⟨1, 1, 1, -352, -2431⟩

/-- LMFDB `(p, a_p)` at the good primes. -/
def apGood : List (ℕ × ℤ) :=
  [(5, 2), (7, 0), (11, -4), (13, 2), (17, -6), (23, -4), (29, -2)]

/-- LMFDB `(p, a_p)` at the multiplicative primes; `a_p = 1` split, `-1` non-split. -/
def apMult : List (ℕ × ℤ) := [(2, 1), (3, -1), (19, -1)]
...
theorem apAdd_certified : ∀ p ∈ apAdd, ∀ h : Fact p.Prime, AdditiveAt p h ∧
  ...
  unfold apAdd at hp
  fin_cases hp
  -- once `apAdd` has a row, continue exactly as in `apMult_certified` (kept out of the proof
