...
def fmt_int(z):
    z = int(z)
    return f"({z})" if z < _sage_const_0  else f"{z}"
...
def emit_full(weight, form_idx=_sage_const_0 ):
    ...
    for j in range(n_pieces):
        ...
        int_qexp = [int(D * (qexp_decomp[n][j] if j < len(qexp_decomp[n]) else _sage_const_0 ))
...
def main():
    ...
    if out_path:
        ...
        stem = base[:-_sage_const_5 ] if base.endswith('.lean') else base  # strip .lean
