...
lemma one_le_finprod {α : Type u} {M : Type v} [CommSemiring M] [PartialOrder M] [IsOrderedRing M]
    {f : α → M} (hf : ∀ i, 1 ≤ f i) : 1 ≤ ∏ᶠ i, f i :=
  finprod_induction _ le_rfl (fun _ _ => one_le_mul_of_one_le_of_one_le) hf

lemma tendsto_geometric_atTop_nhds_zero_of_lt_one {𝕜 : Type u} [Field 𝕜] [LinearOrder 𝕜]
    [IsStrictOrderedRing 𝕜] [Archimedean 𝕜] [TopologicalSpace 𝕜] [OrderTopology 𝕜] {C r : 𝕜}
    (h₁ : 0 ≤ r) (h₂ : r < 1) : Filter.atTop.Tendsto (fun n => C * r ^ n) <| nhds 0 :=
...
@[simp]
lemma ramificationIdx_self {R : Type u} [CommRing R] [IsDedekindDomain R] {p : Ideal R}
    (hbot : p ≠ ⊥) (htop : p ≠ ⊤) : p.ramificationIdx (RingHom.id R) p = 1 := by
  rw [← not_ne_iff, ramificationIdx_ne_one_iff p.map_id.le, map_id]
  exact not_le_of_gt <| pow_lt_self p hbot htop 2 le_rfl

@[simp]
lemma inertiaDeg_self {R : Type u} [Nontrivial R] [CommRing R] {p : Ideal R} (hp : p ≠ ⊤) :
    p.inertiaDeg p = 1 := by
  have : Nontrivial (R ⧸ p) := Quotient.nontrivial hp
  simp [inertiaDeg]
...
noncomputable def realValuation (x : K) : ℝ :=
  (v.valuation K x).casesOn 0 (fun x => (Nat.card <| A ⧸ v.asIdeal : ℝ) ^ Multiplicative.toAdd x)

noncomputable def localDeg : ℕ :=
  v.asIdeal.ramificationIdx (algebraMap A B) w.asIdeal * v.asIdeal.inertiaDeg w.asIdeal

variable (A) in
noncomputable def below : HeightOneSpectrum A :=
  ⟨.comap .., comap_isPrime .., w.ne_bot ∘ IsIntegralClosure.eq_bot_of_comap_eq_bot L⟩
...
lemma localDeg : v.localDeg v = 1 := by
  rw [HeightOneSpectrum.localDeg, Algebra.algebraMap_self,
    ramificationIdx_self v.ne_bot IsPrime.ne_top', inertiaDeg_self IsPrime.ne_top']

lemma below : v.below ℤ ℚ = v := by
  simp [HeightOneSpectrum.below]

lemma absLocalDeg : v.absLocalDeg ℚ = 1 := by
  simp [HeightOneSpectrum.absLocalDeg, below, localDeg]
...
@[ext]
structure _root_.ParMap : Type (max u v) where
...
lemma smul (n : ℤ) (a : A) : p (n • a) = n ^ 2 * p a := by
  ...
  | nat n => induction n using Nat.strongRec with
    | ind n ih =>
      ...
      simp only [Nat.cast_smul_eq_nsmul] at ih ⊢
  ...
  | neg ih => rw [neg_smul, neg, Int.cast_neg, neg_sq, ih]
...
lemma bilinFun_neg_left (a b : A) : p.bilinFun (-a) b = -p.bilinFun a b := by
  rw [eq_neg_iff_add_eq_zero, ← bilinFun_add_left, neg_add_cancel, bilinFun_zero_left]
...
lemma bilinFun_smul_left (n : ℤ) (a b : A) : p.bilinFun (n • a) b = n • p.bilinFun a b := by
  ...
  | nat n => induction n with
    | zero => simp
    | succ _ ih =>
      push_cast [Nat.cast_smul_eq_nsmul, add_smul, one_smul, bilinFun_add_left] at ih ⊢
      rw [ih]
  | neg ih => rw [neg_smul, bilinFun_neg_left, ih, neg_smul]
...
noncomputable def naiveHeight : W.Point → ℝ
  | .zero => 1
  | @Point.some _ _ _ x _ _ =>
    ((∏ᶠ v : FinitePlace K, max 1 (v.maximalIdeal.realValuation x ^ v.maximalIdeal.absLocalDeg K))
      * ∏ᶠ v : InfinitePlace K, max 1 (v x ^ v.mult)) ^ (1 / Module.finrank ℚ K : ℝ)
...
lemma naiveHeight_ge_one (P : W.Point) : 1 ≤ W.naiveHeight P := by
  rcases P with _ | _
  · rfl
  · refine Real.one_le_rpow (one_le_mul_of_one_le_of_one_le ?_ ?_) <| one_div_nonneg.mpr <|
      Nat.cast_nonneg _ <;> exact one_le_finprod fun _ => le_max_left ..
...
theorem naiveHeight_le_finite (C : ℝ) : {P : W.Point | W.naiveHeight P ≤ C}.Finite := by
  ...
  · convert Set.finite_empty using 1
    exact Set.eq_empty_of_forall_notMem fun P => hC ∘ (naiveHeight_ge_one P).trans
...
lemma logHeight_some {x y : K} (h : W.Nonsingular x y) : W.logHeight (.some h) =
    ((∏ᶠ v : FinitePlace K, max 1 (v.maximalIdeal.realValuation x ^ v.maximalIdeal.absLocalDeg K))
      * ∏ᶠ v : InfinitePlace K, max 1 (v x ^ v.mult)).log / (Module.finrank ℚ K : ℝ) := by
  erw [logHeight, Real.log_rpow <| one_pos.trans_le <| one_le_mul_of_one_le_of_one_le ?_ ?_,
    one_div_mul_eq_div] <;> exact one_le_finprod fun _ => le_max_left ..
...
lemma logHeight_smul (n : ℤ) : ∃ C : ℝ, ∀ P : W.Point,
  ...
  | nat n => induction n using Nat.strongRec with
    | ind n ih =>
      ...
      simp only [Nat.cast_smul_eq_nsmul, Int.cast_natCast] at ih ⊢
  ...
  | neg ih => simp_rw [neg_smul, logHeight_neg, Int.cast_neg, neg_sq, ih]
...
@[ext]
structure CanonHeight (A : Type u) [AddCommGroup A] : Type u extends ParMap A ℝ where
...
instance : FunLike (CanonHeight A) A ℝ where
  coe h := h.toFun
  coe_injective' _ _ := CanonHeight.ext
...
lemma exists_min' {C : ℝ} {p : A → Prop} (h : {a : A | p a ∧ H a ≤ C}.Nonempty) :
  ...
  exact ⟨a, hp, hC,
    fun b hb => if hb' : H b ≤ C then ha b ⟨hb, hb'⟩ else le_trans hC (lt_of_not_ge hb').le⟩
...
theorem fg_of_zsmul_coker_finite {n : ℤ} (hn : 1 < |n|) [Finite <| A ⧸ (zsmulAddGroupHom n).range] :
  ...
  rcases (Quotient.out '' (⊤ : Set <| A ⧸ (zsmulAddGroupHom n).range)).exists_max_image H
    (Set.toFinite _) Set.Nonempty.of_subtype with ⟨max, _, hmax⟩
  ...
  have : H min' < H min := lt_of_le_of_lt min'.property <| lt_of_not_ge <| by
    convert AddSubgroup.notMem_of_notMem_closure hmin
  rcases QuotientAddGroup.mk_out_eq_mul (zsmulAddGroupHom n).range min with
  ...
  refine (hmin' c fun h => hmin <| AddSubgroup.zsmul_mem _ h n).not_gt ?_
...
variable {K : Type v} [DecidableEq K] [Field K] [NumberField K] {W : Affine K}
...
lemma canonHeightSeq_sub_succ : ∃ C : ℝ, ∀ P : W.Point, ∀ n : ℕ,
  ...
  rw [abs_sub_comm, canonHeightSeq, pow_succ', mul_smul, canonHeightSeq,
    ...
    mul_le_mul_iff_left₀ <| pow_pos (inv_pos_of_pos four_pos) _, show (4 : ℝ) = 2 ^ 2 by norm_num1]
  exact h <| 2 ^ n • P
...
lemma canonHeightSeq_par_law : ∃ C : ℝ, ∀ P Q : W.Point, ∀ n : ℕ,
  ...
  exact (mul_le_mul_iff_left₀ <| pow_pos (inv_pos_of_pos four_pos) n).mpr <| h ..
...
lemma canonHeight_unique {h : W.Point → ℝ}
  ...
  have hn' : |(|(n : ℝ) ^ 2|⁻¹)| < 1 := by
    simpa only [abs_inv, abs_abs, abs_pow] using inv_lt_one_of_one_lt₀ <| one_lt_pow₀ hn two_ne_zero
...
lemma canonHeight_eq_zero (P : W.Point) : W.canonHeight P = 0 ↔ ∃ n : ℤ, n ≠ 0 ∧ n • P = 0 := by
  ...
  · replace hP : (Set.range fun n => 2 ^ n • P).Finite := hP.subset fun _ ⟨n, hn⟩ => ⟨2 ^ n, hn⟩
    ...
    rw [Real.norm_of_nonneg <| canonHeightSeq_nonneg P n, canonHeightSeq,
      mul_le_mul_iff_left₀ <| pow_pos (inv_pos_of_pos four_pos) n]
