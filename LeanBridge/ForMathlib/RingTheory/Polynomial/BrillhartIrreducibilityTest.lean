import LeanBridge.ForMathlib.FieldTheory.Finite.IrreduciblePolynomialZModp
import LeanBridge.ForMathlib.Algebra.Polynomial.PolynomialsAsLists
...
lemma List.pow_le_prod {R : Type*} [Semiring R] [PartialOrder R] [IsStrictOrderedRing R] (l : List R) (d : R)
...
lemma List.map_maximum {α β: Type*} [LinearOrder α] [LinearOrder β] {l : List α}
  ...
  induction l with
  | nil => simp only [maximum_nil, WithBot.map_bot, map_nil]
  | cons a as ha =>
...
lemma irreducible_of_eval_mul_prime (m : ℤ) (ρ : ℝ) (d p s : ℕ) (hdn : d ≠ 0)
    ...
    (h : ∀ x ∈ Polynomial.roots (P.map (algebraMap ℤ ℂ)), ‖x‖ ≤ ρ)
  ...
  have dvdaux : ∀ a , ¬ IsUnit a → a ∣ P → (|m| - ρ) ^ d ≤ |a.eval m| := by
    ...
    have hla : 1 ≤ ‖(map (algebraMap ℤ ℂ) a).leadingCoeff‖ := by
      ...
      simp [algebraMap_int_eq, eq_intCast, ge_iff_le]
      rw [← Int.cast_one]
      norm_cast
      apply Int.one_le_abs
      exact hazc
    have hrhoaux : ρ ≤ ‖((algebraMap ℤ ℂ) m)‖:= by
...
noncomputable def maxCoeffsAux (P : Polynomial ℂ) : ℝ :=
  let f : Fin (P.natDegree) → ℝ := fun n => ‖(P.coeff n)‖
  (if h : P.natDegree = 0 then 0 else WithBot.unbot (List.maximum (List.ofFn f))
  (List.maximum_ne_bot_of_ne_nil (by simp only [ne_eq, List.ofFn_eq_nil_iff, h, not_false_eq_true,
    f] )))
...
noncomputable def cauchyBound (P : Polynomial ℂ ) : ℝ := 1 + (maxCoeffsAux P) / ‖(P.leadingCoeff)‖
...
lemma polynomial_roots_le_cauchy_bound (P : Polynomial ℂ ) (z : ℂ)
  ...
  have aux1 : ‖(- (P.eraseLead).eval z)‖ ≤
    ∑ i ∈ Finset.range (P.natDegree), (‖(P.coeff i)‖) * (‖z‖) ^ i := by
    rw [norm_neg , Polynomial.eval_eq_sum_range]
    refine le_trans (norm_sum_le  _ _) ?_
    by_cases he : P.eraseLead = 0
    · rw [he]
      simp  [natDegree_zero, zero_add, Finset.range_one, coeff_zero, zero_mul, map_zero,
        Finset.sum_const_zero]
      refine Finset.sum_nonneg ?_
      intro i _
      rw [← norm_pow, ← norm_mul]
      exact norm_nonneg (P.coeff i * z ^ i)
    ...
    · have aux := Polynomial.eraseLead_natDegree_lt_or_eraseLead_eq_zero P
      ...
      simp only [Finset.range_subset_range]
      exact aux
    · intro i _ _
      exact norm_nonneg (P.eraseLead.coeff i * z ^ i)
    · apply Finset.sum_le_sum
      intro i hi
      simp [eraseLead_coeff, ne_of_lt (Finset.mem_range.1 hi), ↓reduceIte, le_refl]
  by_cases habs : 0 ≤  ‖z‖ - 1
  · have aux2 : (∑ i ∈ Finset.range (P.natDegree), (‖(P.coeff i)‖) * (‖z‖) ^ i) * (‖z‖ - 1)
      ≤ (maxCoeffsAux P) * ((‖z‖) ^ P.natDegree - 1) := by
      have aux3 : ∑ i ∈ Finset.range (P.natDegree), (‖(P.coeff i)‖) * (‖z‖) ^ i
        ≤ (maxCoeffsAux P) * ∑ i ∈ Finset.range (P.natDegree), (‖z‖) ^ i := by
        rw [Finset.mul_sum]
        apply Finset.sum_le_sum
        intro i hi
        refine mul_le_mul_of_nonneg_right ?_ (by rw [← norm_pow] ; exact norm_nonneg _ )
    ...
    rw [← sub_le_iff_le_add',
      le_div_iff₀ (norm_pos_iff.mpr (Polynomial.leadingCoeff_ne_zero.2 h)),
      ←  mul_le_mul_iff_of_pos_right (a := (‖z‖ ^ P.natDegree))]
...
noncomputable def maxCoeffsAuxScale (P : Polynomial ℂ) (r : ℝ) : ℝ :=
  let f : Fin (P.natDegree) → ℝ := fun n => ‖(P.coeff n)‖ * (1 / ↑r ^ (P.natDegree - n.val))
  (if h : P.natDegree = 0 then 0 else WithBot.unbot (List.maximum (List.ofFn f))
  (List.maximum_ne_bot_of_ne_nil (by simp only [ne_eq, List.ofFn_eq_nil_iff, h, not_false_eq_true,
    f] )))
...
noncomputable def cauchyBoundScaled (P : Polynomial ℂ) (r : ℝ) : ℝ :=
  r * (1 + (maxCoeffsAuxScale P r) / ‖(P.leadingCoeff)‖)
...
lemma polynomial_roots_le_cauchy_bound_scale (P : Polynomial ℂ) (z : ℂ)
    ...
    ‖z‖ ≤ cauchyBoundScaled P r := by
  have h : P ≠ 0 := Ne.symm (ne_of_apply_ne (fun P => P.natDegree) fun a => hd (id (Eq.symm a)))
  refine le_of_mul_le_mul_of_pos_left (a := r⁻¹) ?_ (inv_pos_of_pos hs)
  have : r⁻¹ * ‖z‖ = ‖(r⁻¹ * z)‖:= by
    rw [ofReal_inv, norm_mul, norm_inv]
    simp [abs_of_nonneg (a := r) (le_of_lt hs)]
...
lemma maxCoeffsAuxScale_ofList (l : List ℤ) (hl : l = l.dropTrailingZeros) (r : ℚ) :
  ...
  · by_cases hlen : l.length - 1 = 0
    ...
    · have hlenc := hlen
      rw [← (natDegree_ofList _ hz hl), add_tsub_cancel_right,
      ← Polynomial.natDegree_map_eq_of_injective (RingHom.injective_int (algebraMap ℤ ℂ))] at hlen
      simp only [maxCoeffsAuxScale, hlen, ↓reduceDIte, coeff_map, eq_intCast, one_div,
...
lemma ofList_roots_le_cauchy_bound_scale (l : List ℤ)
    ...
    ‖z‖ ≤ cauchyBoundScaledOfList l r := by
  ...
  rw [Polynomial.leadingCoeff_map_of_leadingCoeff_ne_zero, algebraMap_int_eq,
    eq_intCast]
  simp [algebraMap_int_eq, eq_intCast, ne_eq, Int.cast_eq_zero, leadingCoeff_eq_zero]
  by_contra hc
