...
/-- If `h₁ * g₁ * (h₂ * g₂) ∈ HdH` (with `h₁, h₂ ∈ H`),
then `⟦d⟧ ∈ mulSupport g₁ g₂`.
...
lemma singleton_one_mul_𝕋 (D2 : HeckeCoset P) (b : ℤ) :
  ...
  exact HeckeCoset.ind fun g ↦ by
    ...
    show Finsupp.mapRange (b • ·) _ (Finsupp.mapRange ((1 : ℤ) • ·) _
        (Finsupp.single ⟦g⟧ (1 : ℤ))) = _
    rw [Finsupp.mapRange_single (hf := smul_zero (1 : ℤ))]
    rw [Finsupp.mapRange_single (hf := smul_zero b)]
    show Finsupp.single ⟦g⟧ (b • (1 : ℤ) • (1 : ℤ)) = Finsupp.single ⟦g⟧ b
...
lemma one_mul_singleton_𝕋 (D2 : HeckeCoset P) (b : ℤ) :
  ...
  exact HeckeCoset.ind fun g ↦ by
    ...
    rw [Finsupp.mapRange_single (hf := smul_zero b)]
    rw [Finsupp.mapRange_single (hf := smul_zero (1 : ℤ))]
    show Finsupp.single ⟦g⟧ ((1 : ℤ) • b • (1 : ℤ)) = Finsupp.single ⟦g⟧ b
    rw [one_smul, smul_eq_mul, mul_one]
    ...
    right_distrib := fun f g h ↦ by
      ...
      · intro D1 _ a b
        ...
        change (Finsupp.sum h fun D2 b₂ ↦ (a + b) • b₂ • m P D1.rep D2.rep) t =
            ((Finsupp.sum h fun D2 b₂ ↦ a • b₂ • m P D1.rep D2.rep) +
              Finsupp.sum h fun D2 b₂ ↦ b • b₂ • m P D1.rep D2.rep) t
