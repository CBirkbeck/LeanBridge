...
/- The localization of a Dedekind domain at a height-one prime is a discrete valuation ring,
provided locally so that the definitions below need not thread it by hand. -/
...
/-- A Weierstrass curve over `FractionRing O` is **globally minimal** (LMFDB
`ec.global_minimal_model`) if it is minimal over the discrete valuation ring
...
def IsGlobalMinimal (W : WeierstrassCurve (FractionRing O)) : Prop :=
  ∀ v : HeightOneSpectrum O, IsMinimal (Localization.AtPrime v.asIdeal) W

/-- A Weierstrass curve over `FractionRing O` is **semi-globally minimal** (LMFDB
...
def IsSemiGlobalMinimal (W : WeierstrassCurve (FractionRing O)) : Prop :=
...
admits a globally minimal model, this is the principal ideal generated by its discriminant. -/
...
/-- A Weierstrass curve over `FractionRing ℤ` is **reduced minimal** (LMFDB
`ec.q.minimal_weierstrass_equation`) if it is globally minimal with `a₁, a₃ ∈ {0, 1}` and
`a₂ ∈ {-1, 0, 1}`. These normalizations single out a unique globally minimal model of an elliptic
curve over `ℚ`. -/
def IsReducedMinimal (W : WeierstrassCurve (FractionRing ℤ)) : Prop :=
  IsGlobalMinimal W ∧
